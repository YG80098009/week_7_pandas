...
df = Loading_a_json_file_into_a_dataframe("orders_simple.json")
    ...
    # Substituting_an_empty_value(df)

    # Create_a_new_column(df)

    # Create_a_new_column_and_sorting(df)

    # Create_a_new_column_by_country(df)

    # Deleting_rows_by_condition(df)

    # Create_a_new_column_base_on_shipping_days(df)

    # Saving_the_table_as_a_csv_file(df)
